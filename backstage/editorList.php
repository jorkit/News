...
<head>
    ...
    <script type="text/javascript">
        ...
        function alldel(){
            ...
            if(f==true){
                if(confirm("您确认要删除吗？"))
                    location.href="../user/userMultidel.php?role_id=2&ids="+arr;
            }else{
                alert("请至少选中一条进行批量删除");
            }
        }
...
<div class="right">
    ...
    <div class="right-content">
        <div class="right-content-top">
            <a href="editorAdd.php"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span>添加</a>
        ...
        <div class="right-content-table">
            <table class="table table-bordered">
                ...
                $countRes = mysqli_query($con,"select count(*) from user where role_id=2");
                $row = mysqli_fetch_array($countRes);
                $count = $row[0];
                $res = mysqli_query($con,"select * from user where role_id=2 limit $offset,$pagesize ");
                mysqli_close($con);
                if(!$res || !$countRes ){
                    die("网站维护中，请下次光临！");
                }
                while($row = mysqli_fetch_array($res,MYSQLI_ASSOC)){?>
                    <tr>
                        ...
                        <td style="vertical-align: middle;">
                            ...
                            <span></span>
                            <span class="glyphicon glyphicon-trash" aria-hidden="true"><a href="../user/userDelete.php?id=<?php echo $row["id"]?>&role_id=2" onclick="javascript:return confirm('是否确认删除')">删除</a></span>
                        </td>
                    </tr>
                    <?php
                } ?>
            ...
            <nav class="navbar navbar-fixed-bottom" style="margin-bottom: 50px">
                <br/>
                <ul class="pager" >
                    <li><a href="editorList.php?pageindex=<?php echo $pageindex-1 < 0 ? 0 : $pageindex-1  ?>">上一页</a></li>
                    ...
                    <li><a href="editorList.php?pageindex=<?php echo $pageindex+1>=ceil($count/$pagesize) ? ceil($count/$pagesize)-1:$pageindex+1 ?>">下一页</a></li>
                </ul>
            </nav>
        </div>
    </div>
